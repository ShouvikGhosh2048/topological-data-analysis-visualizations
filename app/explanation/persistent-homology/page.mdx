...
For this section we will be considering the following example filtration:
<ExampleFiltration/>
...
and drawing arrows representing the function.

(I represent a cycle of edges with their indices)
(TODO: Add attribution to merge trees)
<ExampleFiltrationPersistentFunctionTree/>
...
Now for any i, j, k (i ≤ j), consider img(f<sub>k</sub><sup>ij</sup>).

img(f<sub>k</sub><sup>ij</sup>)

= \{f<sub>k</sub><sup>ij</sup>([z]<sup>i</sup>) | [z]<sup>i</sup> ∈ H<sub>k</sub><sup>i</sup>\}

= \{f<sub>k</sub><sup>ij</sup>([z]<sup>i</sup>) | z ∈ Z<sub>k</sub><sup>i</sup>\}
(the homology group is a set of equivalence classes of cycles, so we can just go over each cycle and choose it's equivalence class)

= \{[z]<sup>j</sup> | z ∈ Z<sub>k</sub><sup>i</sup>\}

Now consider any z, z' ∈ Z<sub>k</sub><sup>i</sup>.

- [z]<sup>j</sup> = [z']<sup>j</sup> iff z' ∈ [z]<sup>j</sup> (equivalence classes are equal or disjoint),
- and z' ∈ [z]<sup>j</sup> iff z - z' ∈ B<sub>k</sub><sup>j</sup> (definition of equivalence class)

Combining the two, we get [z]<sup>j</sup> = [z']<sup>j</sup> iff z - z' ∈ B<sub>k</sub><sup>j</sup>.

Since z, z' ∈ Z<sub>k</sub><sup>i</sup>, z - z' ∈ Z<sub>k</sub><sup>i</sup>.
Thus z - z' ∈ B<sub>k</sub><sup>j</sup> iff z - z' ∈ Z<sub>k</sub><sup>i</sup> ∩ B<sub>k</sub><sup>j</sup>.
This is useful as Z<sub>k</sub><sup>i</sup> ∩ B<sub>k</sub><sup>j</sup> is a subspace of Z<sub>k</sub><sup>i</sup>,
and so we can consider the quotient.

Thus we consider Z<sub>k</sub><sup>i</sup> / (Z<sub>k</sub><sup>i</sup> ∩ B<sub>k</sub><sup>j</sup>).
For any equivalence class, all z in the class have the same [z]<sup>j</sup>'s, and for different equivalence classes,
the [z]<sup>j</sup>'s will be different.

Thus we get a bijection from Z<sub>k</sub><sup>i</sup> / (Z<sub>k</sub><sup>i</sup> ∩ B<sub>k</sub><sup>j</sup>)
to img(f<sub>k</sub><sup>ij</sup>). This bijection can be shown to be linear.

Thus dim(img(f<sub>k</sub><sup>ij</sup>)) = dim(Z<sub>k</sub><sup>i</sup> / (Z<sub>k</sub><sup>i</sup> ∩ B<sub>k</sub><sup>j</sup>)).
This is defined as the **[persistent Betti number](https://en.wikipedia.org/wiki/Persistent_Betti_number) β<sub>k</sub><sup>ij</sup>**.
...
In general if you consider the homology classes H<sub>k</sub><sup>i</sup> for F<sub>i</sub>, and follow them down the tree, the classes at each row form a vector space,
...
A positive simplex s doesn't create any new boundaries when added. It does create new cycles.

All the new cycles created on adding s will contain s as an element, since any cycle not containing s would have been present before. Thus every non-zero cycle has a positive simplex.

We can show every positive simplex has a cycle where it's the only positive simplex.
We do this by going through the sequence:
...
at the end of the step, the homology classes of the cycles in the lists will form bases of the homology groups 
...
- If s is a negative k-simplex, let F<sup>1</sup> and F<sup>2</sup> be the simplicial complexes before and after adding s.
    ...
    - Since the cycles in the lists represent bases of homology groups and [b]<sup>1</sup> ≠ [0]<sup>1</sup>,
    ...
    so [c<sub>1</sub>]<sup>2</sup> + ... + [c<sub>m</sub>]<sup>2</sup> = [0]<sup>2</sup>.
    ...
    - For the cycles in the sum, consider their corresponding positive simplices, and choose the simplex which comes last in the simplex sequence.
...
### 2.5. Persistence Diagram
Generally the filtration F<sub>1</sub>, ..., F<sub>n</sub>, has a value v<sub>i</sub> associated with each F<sub>i</sub> - this v<sub>i</sub> generally represents a parameter which can varied. 
(For example, for the filtration of Vietoris Rips complexes we saw in the first example, we could associate each complex with it's corresponding d).

Then we can consider the multiset P obtained by taking the point (v<sub>i</sub>, v<sub>j</sub>) c<sub>ij</sub> number of times. Each (v<sub>i</sub>, v<sub>j</sub>)
represents a positive simplex born at parameter value v<sub>i</sub> which is paired with a negative simplex born at v<sub>j</sub> (they represent a birth-death pair).
